...
public class DPRProductProcessor {
    ...
    private void processHeader(RadarRecord record, DPRHeaderData headerData) {
        ...
        short precipDetectedFlag = HydroNumericUtility
                .getHighOrderByte(halfWord30);
    ...
    private void processGriddedDataProduct(RadarRecord record,
            DPRHeaderData headerData) {

        // DAA and DSA product processing use: byte[] dataArray =
        // record.getRawData();
        ...
        if (rawShortDataArray == null) {
            ...
            float[][] polarGrid = getPolarGridFromDataArray(rawShortDataArray,
    ...
    private float[][] getPolarGridFromDataArray(short[] dataArray, float scale,
        ...
        for (int r = 0; r < numRadials; r++) {
            for (int b = 0; b < numRangeBins; b++) {
                ...
                if (shortValue == NO_DATA_FLAG) {
                    ...
                    // inches to mm
                    floatValue *= 25.4f;// change units from hundredths of
                                        // inches to mm
                }
    ...
    private String getProductObsTimeString(RadarRecord record,
        ...
        // prodObsDate and prodObsTime for DPR products are in a different
        // location in the
    ...
    private void writeToDPRRadarTable(DPRHeaderData d) {
        ...
        writeToDPRRadarTable(d.getRadarId(), d.getObsTime(),
                ...
                d.getScale(), d.getOffSet(), d.volumeScanDate,
    ...
    private void writeToDPRRadarTable(String radid, String obstime,
            ...
            float scale, float offset, int volumeScanDate, int volumeScanTime,
            short biasValue, short precipDetectedFlag, String filename) {
        ...
        DPRRadar radarObject = new DPRRadar(id, volumeCoveragePattern,
                operationalMode, maxVal, scale, offset, volumeScanDate,
                volumeScanTime, biasValue, precipDetectedFlag, filename);
    ...
    private void writeOutDataFile(RadarRecord record, DPRHeaderData headerData,
        ...
        try {
            ...
            /*
             * leave for future use float max = -9999f;
             */
            ...
            for (int x = 0; x < hrapXMax; x++) {
                for (int y = 0; y < hrapYMax; y++) {
                    ...
                    /*
                     * leave for future use if (precipGridArray[x][y] > max) {

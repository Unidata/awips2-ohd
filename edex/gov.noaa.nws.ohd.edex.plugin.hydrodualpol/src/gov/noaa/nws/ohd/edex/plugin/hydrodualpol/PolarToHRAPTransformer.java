...
/**
 * Class to handle conversion from Polar coordinates to HRAP coordinates for
 * DAA, DSA, and DPR product processors for MPE and HPE/HPN.
 * <p>
 * 
 * <pre>
 * SOFTWARE HISTORY
...
public class PolarToHRAPTransformer {
    ...
    static final int KBINS = 920; /*
                                   * # of bins in the 232KM polar grid --
                                   * original value was 115
                                   */
    ...
    static final double AZ_RND = 1.05; // Factor used to convert azimuth angle
                                       // to azimuth
    ...
    private void build_lfm_lookup(double radarLat, double radarLon) {
        double b, r, ls, lamdas, pre_gxs, gis, gjs, gi, gj, rl, cos_dlamda, sin_dlamda, sin_s, cos_s, sin_ls, cos_ls, sin_l, cos_l, sin_b, cos_b, cos_lamdas_prime, sin_lamdas_prime;
        ...
        // in A2, we receive the radarLon as negative, so we had to remove the
        // A1 correction
        // 7/19/13 cg and pt
        ...
        for (iaz = 0; iaz < MAX_AZMTH; iaz++) {
            ...
            for (irg = 0; irg < KBINS; irg++) {
                ...
                gi = rl
                        * (sin_dlamda * cos_lamdas_prime + cos_dlamda
                                * sin_lamdas_prime) + ip;
                gj = rl
                        * (cos_dlamda * cos_lamdas_prime - sin_dlamda
                                * sin_lamdas_prime) + jp;
                ...
                /**
                 * If lfm coordinates are within local gridi, then save it into
                 * the lookup table and set the 1/40 lfm box range number table
                 * to within range
                 */
                if ((lfm_i > 0) && (lfm_i <= HYZ_LFM40) && (lfm_j > 0)
                        && (lfm_j <= HYZ_LFM40)) {
                    ...
                    lfm40flag[FLAG_RNG][lfmbox] = WITHIN_RANGE;

                    // below is commented out due to test change
        ...
        /*
         * Search for holes in the 1/40 lfm lookup table and identify the az/ran
         * of the data to be used to fill in the hole
         */
        for (i = 0; i < NUM_LFM40; i++) {

            /*
             * Holes exist where lfm40flag[FLAG_RNG][i] contains the initial
             * value
             */
            if (lfm40flag[FLAG_RNG][i] == BEYOND_RANGE) {
                ...
                AzimuthAndRangePoint point = lfm_to_azran(ls, lamdas,
                        lfm40_idx, lfm_i + 1, lfm_j + 1);
                az_ij = point.getAzimuth();
                rg_ij = point.getRange();

                /*
                 * If the range is within the product coverage area requirement
                 * then store azimuth and range index into the lfm40flag lookup
                 * table
                 */
                if (rg_ij <= LFM40_RNG) {
    ...
    private AzimuthAndRangePoint lfm_to_azran(double ls, double lamdas,
            int lfmsize, int lfm_i, int lfm_j) {

        AzimuthAndRangePoint point = new AzimuthAndRangePoint();
        double theta_cij;
        double rg_ij;

        double pre_gxs, gis, gjs, cos_lamdas_prime, sin_lamdas_prime, sin_dlamda;

        double[] ai = new double[3];
        double[] aj = new double[3];

        double cii, cjj, sin_ls, cos_ls, aa, bb, cos_ss, sin_ss, cos_lij, sin_lij, lamda_ij, l_ij;
        ...
        /* If angle is less than 0 then add 360 degrees */
        if (theta_cij < ZERO) {
    ...
    public float[] transformHrapPrecipToDBA(float[] hrapGrid,
            float outputNoDataFlag) {
        ...
        for (int x = 0; x < HYZ_LFM40; x++) {
            for (int y = HYZ_LFM40 - 1; y > -1; y--) {
                ...
                if (hrapGridValue == BEYOND_RANGE) {
                    ...
                    dbaArray[k] = (float) (10.0 * Math
                            .log10((hrapGridValue * .254)));
                }
    ...
    public float[] transform250MeterPolarToFullHRAP(float[][] polarGrid,
        ...
        for (j = 0; j < MAX_AZMTHS; j++) {
            ...
            /*---------------------------------------------------------------------*/
            /*
             * DO UNTIL THE END OF THE RADIAL OR THE GRID DOMAIN HAS BEEN
             * EXCEEDED
             */
            /*---------------------------------------------------------------------*/

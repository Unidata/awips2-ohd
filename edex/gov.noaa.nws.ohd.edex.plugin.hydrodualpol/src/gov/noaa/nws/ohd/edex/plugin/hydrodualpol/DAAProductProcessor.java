...
public class DAAProductProcessor {
    ...
    private void processNullProduct(RadarRecord record, DAAHeaderData headerData) {
        ...
        if (!withinTOHWindow) {
            ...
            // compare null product date/time with end date/time (obstime) of
            // product
            // if time diff is > daa_min_coverage_dur, then set decoded product
            // to all 0.0
            // else set decoded product to all missing
            // if null product string does not have a date/time, then set
            // decoded field to all missing
            ...
            if (nullProductDateTimeString != null) {
                ...
                long nullProductTimeInMillis = HydroTimeUtility
                        .getMillisFromAmericanDateTimeString(nullProductDateTimeString);
                long obsTimeInMillis = HydroTimeUtility
                        .getMillisFromSQLString(obsTimeString);

                /*
                 * leave for future changes long minCoverageInMillis =
                 * minimumCoverageDuration * 60 * 1000;
                 */
                long millisSincePrecipDetected = obsTimeInMillis
                ...
                // TODO
                /*
                 * The following commented-out code is a placeholder for future
                 * changes anticipated from //CCR NA12-00264 which will add the
                 * reporting of the total number of minutes of operation (<=60)
                 * //for which the product applies
                 * 
                 * if(millisSincePrecipDetected >= minCoverageInMillis) {
                 * 
                 * //set decoded field to all 0.0 } else {
                 * 
    ...
    /**
     * Process NULL product String when read from SymbologyBlock
     * 
     * @param headerData
     * @param record
    ...
    private String processNullProductString(DAAHeaderData headerData,
            RadarRecord record) {
    ...
    private String getProductGenerationTimeString(RadarRecord record) {
        ...
        String productGenerationDateTimeString = format
                .format(productGenDateTime);
    ...
    private void processGriddedDataProduct(RadarRecord record,
        ...
        if (useTimeWindowFilter) {
            withinTOHWindow = TopOfHourCheck(decodeWindowInMinutes, obsTime,
                    minOff);
        } else {
            withinTOHWindow = true;
        }
        ...
        if (!withinTOHWindow) {
            ...
            // If current product and a previous product are same number of
            // minutes off TOH, then
            // current product is considered closer and is processed

            boolean closerToTOH = CloserToTOHCheck(decodeWindowInMinutes,
            ...
            if (!closerToTOH) {
                ...
                if (dataArray == null) {
                    ...
                    float[][] polarGrid = getPolarGridFromDataArray(dataArray,
                    ...
                    float[] hrapGrid = transformer
                            .transform250MeterPolarToFullHRAP(polarGrid, lat,
                                    lon);

                    float[] dbaArray = transformer.transformHrapPrecipToDBA(
                            hrapGrid, DECODED_DAA_NO_DATA_FLAG);
                    ...
                    String fileName = String.format("%s%s%02d%02dZ", radid,
                            HydroTimeUtility.JulianDateConvertToMDY(prodDate),
                            prodHour, prodMin);
    ...
    private boolean CloserToTOHCheck(int decodeWindowInMinutes, String radid,
            String obsTime, short minOff) {

        short tohHour, afterHour, beforeHour;
        String obsHourString, tohDate, afterDate, beforeDate, tohDateTime, dateTime;

        // Check if current product is closer to TOH than a previous product
        // deemed to be TOH
        // If current product is closer to TOH, then process it
        // Else do not process the current product
        // If current product and a previous product are same number of minutes
        // off TOH, then
        // current product is considered closer and is processed
        // If current product is exactly at TOH (minoff = 0), then process it
        // (i.e. return true)

        // This check is for decoding only (i.e. not for archive)

        // Read records from the DAARadar table to determine if previous
        // products for current hour
        // have been processed

        // if hour of current product = 0, then products from before the TOH
        // will have previous day's date with hour = 23
        // if hour of current product = 23, then products from after the TOH
        // will have next day's date with hour = 0
        ...
        // search DAARadar table for records before the TOH and after the TOH
        // and within the TOH window
        // for the given radar
        // compare the minoff values for each product to determine if current
        // product is closer to the TOH
        // than a previous product
        // if minOff = 1, then fall through for loop and return true

        short aminOff = minOff;
        if (aminOff < 0) {
            aminOff = (short) (-1 * minOff);
        }
        ...
        for (int i = 1; i < aminOff; i++) {
            ...
            dateTime = String
                    .format("%s %02d:%02d:00", afterDate, afterHour, i);
            ...
            int ib = 60 - i;
            dateTime = String.format("%s %02d:%02d:00", beforeDate, beforeHour,
                    ib);
    ...
    // ---------------------------------------------------------------------
    private void writeToDAARadarTable(float maxValueFromData, String fileName,
            DAAHeaderData d) {

        writeToDAARadarTable(d.getRadarId(), d.getObsTime(),
                d.getMinutesOffTopOfHour(), d.getMaxValh(), maxValueFromData,
    ...
    private boolean TopOfHourCheck(int window, String obsTime, short minOff) {
        ...
        aminoff = minOff;
        if (aminoff < 0) {
            aminoff = (short) (-1 * aminoff);
        }

        if (aminoff <= window) {
            return true; // product is within top-of-hour (TOH) window
        } else {
            return false; // product not within window around TOH
        }
    }
    ...
    // keep the following method for future use

    private void printNonZeroRadialInDataArray(byte[] dataArray,
            int numRadials, int numRangeBins, String fileName) {
        int index = -0;
        ...
        for (int r = 0; r < numRadials; r++) {
            for (int b = 0; b < numRangeBins; b++) {
                ...
                if (!foundRadial) {
                    if (byteValue > 0) {
                        nonZeroRadial = r;
                        printRadial(dataArray, nonZeroRadial, numRadials,
                                numRangeBins, fileName);
                        foundRadial = true;
                        break;
                    }
                }
    ...
    private void printRadial(byte[] dataArray, int radialIndex, int numRadials,
    ...
    // ---------------------------------------------------------------------
    private void writeOutDataFile(RadarRecord record, float[] dbaArray,
            String fileName) {
        ...
        try {
            ...
            for (int i = 0; i < totalGridBins; i++) {
                int intValue = HydroNumericUtility
                        .getSwappedIntBytesFromFloat(dbaArray[i]);
    ...
    private void writeToDAARadarTable(String radid, String obstime,
            short minOff, float maxValh, float maxVald, float s1BiasValue,
            String productTime, short nullProductFlag, int coverageDur,
            String gridFilename) {
        ...
        Date productTimeDate = HydroTimeUtility
                .getDateFromSQLString(productTime);
        DAARadar radarObject = new DAARadar(id, minOff, maxValh, maxVald,
                s1BiasValue, productTimeDate, nullProductFlag, coverageDur,
                gridFilename);
        try {
            CoreDao dao = new CoreDao(DaoConfig.forDatabase("ihfs"));

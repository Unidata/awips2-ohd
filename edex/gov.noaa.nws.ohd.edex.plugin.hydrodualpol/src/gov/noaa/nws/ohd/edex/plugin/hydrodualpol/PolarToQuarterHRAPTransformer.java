...
/**
 * Class to handle conversion from Polar coordinates to Quarter HRAP coordinates
 * for DSA, and DPR product processors for HPE/HPN.
 * <p>
 * 
 * <pre>
 * SOFTWARE HISTORY
...
public class PolarToQuarterHRAPTransformer {
    ...
    private static final double RNG_INC = 0.25; /*
                                                 * for dual pol DSA/DPR product
                                                 * with 0.25 km resolution
                                                 */
    ...
    public PolarToQuarterHRAPTransformer(Logger statusHandler) {
    ...
    void build_lookup_table(double radar_lat, double radar_lon) {
         ...
         * center of 524x524 HRAP grid which is the center of radar, this value
         * changes when the HRAP grid size changes.
         */
        ...
        for (iaz = 1; iaz <= MAX_AZIMUTH; iaz++) {
            ...
            for (irg = 1; irg <= MAX_RANGE_BINNUM; irg++) {
                ...
                cos_delta_lamda = Math.sqrt(ONE - sin_delta_lamda
                        * sin_delta_lamda);

                rl = R2KO * cos_lat / (ONE + sin_lat);

                grid_i = rl
                        * (sin_delta_lamda * cos_lambas_prime + cos_delta_lamda
                                * sin_lamdas_prime) + GRID_COORD_I;
                grid_j = rl
                        * (cos_delta_lamda * cos_lambas_prime - sin_delta_lamda
                                * sin_lamdas_prime) + GRID_COORD_J;
                ...
                if ((lfm_i > 0) && (lfm_i <= MAX_IHRAP) && (lfm_j > 0)
                        && (lfm_j <= MAX_JHRAP)) {
        ...
        // checker.writeIntegerGrid(radar_to_quarter_hrap_i, MAX_RANGE_BINNUM,
        // MAX_AZIMUTH, outputDir + "radar_to_quarter_hrap_i.txt" );
        // checker.writeIntegerGrid(radar_to_quarter_hrap_j, MAX_RANGE_BINNUM,
        // MAX_AZIMUTH, outputDir + "radar_to_quarter_hrap_j.txt" );

        // checker.writeIntegerGrid(quarter_hrap_to_radar_azimuth, MAX_IHRAP,
        // MAX_JHRAP, outputDir + "quarter_hrap_to_radar_azimuth.txt" );
        // checker.writeIntegerGrid(quarter_hrap_to_radar_range, MAX_IHRAP,
        // MAX_JHRAP, outputDir + "quarter_hrap_to_radar_range.txt" );
        ...
        for (ihrap = 0; ihrap < MAX_IHRAP; ihrap++) {
            for (jhrap = 0; jhrap < MAX_JHRAP; jhrap++) {
                ...
                if ((quarter_hrap_to_radar_azimuth[ihrap][jhrap] == BEYOND_GRID)
                    ...
                    /*
                     * calculate the azimuth AZ_IJ and range RG_IJ for the hrap
                     * point (lfm_i, lfm_j)
                     */
                    AzimuthAndRangePoint point = quarterhrap_to_az_range(LS,
                            LAMDA, lfm_i, lfm_j);
                    ...
                    /*
                     * if the range is within the product coverage area (for DSA
                     * product the maximum range length is 230 km) requirement
                     * then store azimuth and range index into the lookup table
                     */
                    ...
                    if (RG_IJ < MAX_RANGE_LENGTH) {
                        ...
                        /*
                         * fill the hole's azimuth with (AZ_IJ+AZ_RND), range
                         * with (RG_IJ/RG_RND+ONE)
                         */
                        quarter_hrap_to_radar_azimuth[ihrap][jhrap] = (int) (AZ_IJ + AZ_RND);
                        quarter_hrap_to_radar_range[ihrap][jhrap] = (int) (RG_IJ / RG_RND)
                                + ONE;
                    } else /*
                            * assign BEYOND_RANGE when RG_IJ is out of the
                            * maximum range length
                            */
                    {
                        quarter_hrap_to_radar_azimuth[ihrap][jhrap] = BEYOND_RANGE;
                        quarter_hrap_to_radar_range[ihrap][jhrap] = BEYOND_RANGE;
                    }
                }
            }
        }
    ...
    AzimuthAndRangePoint quarterhrap_to_az_range(double LS, double LAMDAS,
        ...
        CJJ = AJ + B_CON * LFM_J;
        L_IJ = DTR * NINTY - TWO
                * Math.atan(Math.sqrt(CII * CII + CJJ * CJJ) / R2KO);
        ...
        BB = COS_LS * SIN_LIJ - SIN_LS * COS_LIJ
                * Math.cos(LAMDA_IJ - DTR * LAMDAS);
    ...
    float[][] transform250MeterPolarToQuarterHRAP(float[][] polarGrid,
        ...
        /*
         * convert DSA product storm-total precipitation into appropriate scale
         * (0.01 inch) in mm unit, do not use the offset/scale format when the
         * initial raw data is 0 which represents NO Data
         */
        ...
        for (int i = 0; i < MAX_IHRAP; i++) {
            for (int j = 0; j < MAX_JHRAP; j++) {
                if (num_bin_1km[i][j] > 0) {
                    radar_1km[i][j] = sum_rad_1km[i][j] / num_bin_1km[i][j];

                    // System.out.println("radar_1km[" + i +"][" + j +"] = " +
                    // radar_1km[i][j]);

                    // logger.debug("DPR product: sum_rad_1km[" + i +"][" + j
                    // +"] = " + sum_rad_1km[i][j]);
